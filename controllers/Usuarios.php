...
class Usuarios
{
    public static function nuevo($correo, $username, $pass)
    {
        $bd = BD::obtener();
        $sentencia = $bd->prepare("insert into usuarios(correo, username, pass) VALUES (?, ?, ?);");
        $pass = password_hash(md5($pass), PASSWORD_DEFAULT);
        $resultadoInsercion = $sentencia->execute([$correo, $username, $pass]);
...
values ('$idUsuario', '', '', '', '');";
    ...
    {
        $bd = BD::obtener();
        $sentencia = $bd->prepare("select id, correo, pass, username from usuarios where correo = ? limit 1;");
    ...
    {
        $bd = BD::obtener();
        $sentencia = $bd->prepare("select id, correo, username from usuarios where id = ? limit 1;");
